...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (37016142,4180113,4324190,4212564,4178782,35610791,4197758,4180131,4187203,4190633,4333345,4187205,4333465,4180749,46273501,43021271,4179069,4187206,4325868,4216132,4323346,4179084,4325186,4181024,46270077)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (37016142,4180113,4324190,4212564,4178782,35610791,4197758,4180131,4187203,4190633,4333345,4187205,4333465,4180749,46273501,43021271,4179069,4187206,4325868,4216132,4323346,4179084,4325186,4181024,46270077)
  and c.invalid_reason is null
...
) A on A.person_id = P.person_id  AND A.START_DATE >= DATEADD(day,-1826,P.START_DATE) AND A.START_DATE <= DATEADD(day,0,P.START_DATE) ) cc 
...
select C.person_id, C.drug_exposure_id as event_id, C.drug_exposure_start_date as start_date,
       COALESCE(C.DRUG_EXPOSURE_END_DATE, DATEADD(day,C.DAYS_SUPPLY,DRUG_EXPOSURE_START_DATE), DATEADD(day,1,C.DRUG_EXPOSURE_START_DATE)) as end_date,
       C.visit_occurrence_id,C.drug_exposure_start_date as sort_date, C.drug_concept_id as domain_concept_id
from 
(
  select de.* 
  FROM @cdm_database_schema.DRUG_EXPOSURE de
